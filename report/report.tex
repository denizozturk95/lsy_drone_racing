...
%% External image files needed (place next to this .tex): speed_profile.png (Fig. 2),
%% racing_line_sim.png and racing_line_3d.png (Fig. 5, left and right). Other figures are
%% self-contained. Compile with pdflatex, run twice.
...
\label{eq:bwd}
\end{equation}
The result is a speed that is high on the straights and low at the gates and corners, shown in Fig.~\ref{fig:speed}. Finally we integrate the travel time along the line and resample the position, velocity and acceleration on a 50~Hz time grid. These three arrays are the reference the tracker follows.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{WhatsApp Image 2026-07-05 at 20.06.21.jpeg}
\caption{Planned speed profile the tracker follows, against time since the launch hand-off. The speed rises on the straights and drops at each gate, marked G1 to G4, and at each tight turn, set by the shared acceleration budget of \eqref{eq:curv}--\eqref{eq:bwd}. It stays under the top-speed cap $v_{\text{ceil}} = 3.8$ m/s (dotted). The planned lap is about 6.35~s with a peak speed of 3.62~m/s.}
\label{fig:speed}
\end{figure}
...
\subsection{Setup}
We evaluate on the \texttt{final.toml} competition track of Section~\ref{sec:problem}, the fixed-layout course that every group shares. Its four gates, its four obstacles and the racing line through them are shown in Fig.~\ref{fig:racingline}. The line reverses direction twice, once near the third gate and once before the last gate, which set the pace. The reported numbers are from the official evaluation with seed 2026, which draws a fresh disturbance for each of the 20 runs. Table~\ref{tab:params} lists the main parameters. They are tuned once offline and are held fixed across all runs.

% Track schematic removed: the simulation figure (Fig.~\ref{fig:racingline}) now shows the
% track layout and the racing line.
...
On the official evaluation the controller passes all 20 runs with a lap time of 6.89~s. Fig.~\ref{fig:racingline} shows the racing line. The top-down view threads each gate near the center and stays clear of the obstacle keep-outs, and it runs slow at the gates and the tight turns and fast on the straights. The three-dimensional view shows the gates at two heights, so the drone climbs and descends between them. The controller holds the 50~Hz rate with a wide margin, since it runs no solver and only evaluates the warp and the tracking law each step.

\begin{figure*}[t]
\centering
\includegraphics[width=0.45\textwidth]{WhatsApp Image 2026-07-05 at 20.06.16.jpeg}
\hfill
\includegraphics[width=0.54\textwidth]{WhatsApp Image 2026-07-05 at 20.06.10.jpeg}
\caption{Racing line of the controller on the competition track (\texttt{final.toml}), from the simulator. Left: seen top down and colored by the planned speed, set by the profile of Section~\ref{sec:speed}; the line is slow (blue to green) through the gates and the tight turns and fast (orange to red) on the straights. Green bars are the gate openings with the crossing direction, red dots are obstacles with their keep-out circles and the star is the start. Right: the same line in three dimensions, where the gates (green) sit at two heights so it climbs and descends, and the obstacles (red) are full-height poles it routes around. The planned lap is about 6.35~s and the tracker completes it in 6.89~s in simulation.}
\label{fig:racingline}
\end{figure*}

\subsection{Comparison with the Optimization Controller}
Before this controller we built an online MPCC controller for the same track, and we flew it on the real Crazyflie in the last lab session (Section~\ref{sec:deploy}). Table~\ref{tab:compare} compares the two in simulation. The optimization controller reaches 9.55~s, finishes 17 of 20 runs and needs a compiled solver in the loop. The feedforward tracker is 2.66~s faster and passes all 20 runs, and it has no solver, so it is simpler to run and to reason about. The gain does not come from a faster vehicle. Both use the same drone and the same acceleration limits. It comes from where the effort goes. The optimization controller spends the run re-solving a hard problem near each gate, while the tracker spends it holding a fixed line with a clock that waits when needed.

\begin{table}[htbp]
\caption{Comparison on the competition track}
\begin{center}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Controller} & \textbf{Solver} & \textbf{Success} & \textbf{Lap (s)} \\
\hline
Online MPCC & yes & 17/20 & 9.55 \\
\hline
...
\label{sec:deploy}
This line of work has already flown on real hardware. In the last lab session we flew the predecessor of the tracker, an online MPCC controller, on the physical Crazyflie under the Vicon system. It completed the course at 9.55~s, the same lap time it reached in simulation. Its behavior on the drone matched the simulator, which tells us that the simulator we use is a good proxy for the real vehicle on this track.
