...
\usepackage{graphicx}
\graphicspath{{figs_v24_r10/}}  % robust v24_r10 / final.toml figure set
\usepackage{textcomp}
\usepackage{xcolor}
\usepackage{tikz}
...
This report comes out of a course project built as a ladder of levels, where each level switches on more disturbance than the one below. We worked through the lower levels first, on a known track and then under inertial and pose randomization, and each result carried into the next, so the top of the ladder reuses a racing core that the lower levels validated. Level 3 is that top. It randomizes the whole layout, which makes it the hardest and most general level and the end product of the project. The graded evaluation runs on one shared track, \texttt{final.toml}, so all groups are compared on the same course. We treat that course as one more Level 3 draw and never read its nominal layout, which is what our design is built to do.
...
\FORALL{gates $j$}
    \IF{$j \notin K$ and $\lVert p_{xy} - j_{xy} \rVert < 0.7$}
        \STATE $K \leftarrow K \cup \{j\}$ \quad (true pose revealed)
    \ENDIF
\ENDFOR
...
\fill (pr) circle (1.4pt); \node[below] at (pr) {$p_{\text{ref}}(\bar\xi)$};
...
Table~\ref{tab:results} reports the controller across settings and tracks. On randomized Level 3 the robust setting finishes 12 of 20 runs at a lap time of 18.8~s. The fast setting raises the speed budget and tightens the search, and it cuts the lap time to 12.8~s at 10 of 20 runs. The fast setting trades about ten points of success for roughly a third off the lap time, which is the aggressiveness dial at work. On the fixed competition track the robust setting finishes all 20 runs at 25.8~s. The success rate is higher there because the fixed layout is feasible on every draw, while the randomized draws sometimes place gates and obstacles in tight or crossing configurations that the racing stage cannot thread.
...
\hline
Robust & \texttt{final.toml} (fixed) & 20/20 & 25.78 \\
\hline
\end{tabular}
\label{tab:results}
\end{center}
...
The lap time is dominated by the search, not by the racing. The spiral has to cover the arena before every gate is seen, and this takes on the order of 20~s on the fixed track. The racing itself, once the gates are known, takes only a few seconds. Fig.~\ref{fig:speed} shows this split in the speed trace: a long low-speed sweep while the arena is covered, then a short fast burst through the four gates. This is why the controller reaches 25.8~s on a track that a known-track racer flies in under 10~s. The gain in return is generality. Because the search never reads the nominal layout, the same controller runs on any Level 3 draw, which is what the setting asks for.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_speed.png}
\caption{Speed against time on the fixed competition track. The long low-speed section is the blind spiral search covering the arena, and the short high-speed section is the race through the four discovered gates. The search, not the racing, sets the lap time.}
\label{fig:speed}
\end{figure}
...
\section{Conclusion}
We presented a general Level 3 controller for autonomous drone racing under randomized layouts. It finds the gates with a blind spiral sweep that does not use the nominal positions, then races them with a time-optimal MPCC that makes speed an output. A gate-aware contouring weight and a dynamics-aware progress anchor keep the racing stage reliable. The controller finishes 12 of 20 runs on randomized Level 3 at 18.8~s and all 20 runs on the fixed competition track at 25.8~s. Its racing core is the same one we flew on the real drone at 9.55~s in a fast known-track configuration. The design trades lap time for generality, which is the trade the full Level 3 setting demands.
