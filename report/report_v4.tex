...
We study the Level 3 setting of the LSY Drone Racing competition. Gate and obstacle positions are drawn at random, and the true pose of an object is reported only when the drone is within 0.7~m of it. For most of the competition the reported nominal was a placeholder at the origin, so a controller that relied on it could not fly the track. A late change to the environment began reporting a coarse nominal, within about 15~cm of the truth, which is too coarse to thread a gate at speed but enough to plan a route (Section~\ref{sec:nosearch}). Our controller does not depend on the nominal layout at all. It finds the gates by flying and racing them once they are found.
...
\label{sec:settings}
We use two settings of the same controller, labeled by their configuration names. Both run the same blind search and the same MPCC, and neither reads the nominal layout, so the difference is aggressiveness and not method. The \texttt{v24\_r10} setting keeps a moderate speed budget so that it finishes on as many random draws as possible, and it leads on fully randomized Level 3. The \texttt{v24\_r10\_fast} setting is more aggressive: it raises the MPCC speed budget and tightens the search, so it reaches the gates sooner and races them harder. On randomized Level 3 it cuts the lap time by about a third, from 18.8~s to 12.8~s, at a small cost in success rate. On the larger \texttt{final.toml} arena, where the slower \texttt{v24\_r10} spiral more often times out, \texttt{v24\_r10\_fast} is both quicker and more reliable, so it is our choice for the graded track. The two settings differ only in these parameters, so this is an aggressiveness choice and not a change of method. Table~\ref{tab:results} reports both.
...
Table~\ref{tab:results} reports the controller across settings and tracks. Each run draws a fresh random layout, so both the \texttt{level3.toml} and \texttt{final.toml} columns measure generalization across 20 distinct layouts, not one layout under disturbance. On fully randomized Level 3 the \texttt{v24\_r10} setting finishes 12 of 20 runs at 18.8~s, and \texttt{v24\_r10\_fast} cuts the lap time to 12.8~s at 10 of 20, trading about ten points of success for a third off the lap time. On the graded \texttt{final.toml} draws the order reverses: \texttt{v24\_r10\_fast} finishes 14 of 20 at 18.4~s while \texttt{v24\_r10} finishes only 9 of 20 at 26.9~s. \texttt{final.toml} uses a larger arena than \texttt{level3.toml}, so the slower \texttt{v24\_r10} spiral must cover more ground and more often times out before every gate is seen; the faster sweep does not, which is why \texttt{v24\_r10\_fast} both wins and is our headline result on the graded track. The remaining misses on either track come from draws that place gates and obstacles in tight or crossing configurations that the racing stage cannot thread.
...
\begin{table}[htbp]
\caption{The controller across settings and tracks. All tracks are randomized; each row is at least 20 runs over distinct layouts. The last row is the no-search variant.}
\begin{center}
\begin{tabular}{|l|l|c|c|}
\hline
\textbf{Setting} & \textbf{Track} & \textbf{Success} & \textbf{Lap (s)} \\
\hline
\texttt{v24\_r10} & \texttt{level3.toml} & 12/20 & 18.83 \\
\hline
\texttt{v24\_r10\_fast} & \texttt{level3.toml} & 10/20 & 12.81 \\
\hline
\texttt{v24\_r10} & \texttt{final.toml} & 9/20 & 26.91 \\
\hline
\texttt{v24\_r10\_fast} & \texttt{final.toml} & 14/20 & 18.36 \\
\hline
\texttt{v24\_r10\_nosearch} & \texttt{final.toml} & 11/20 & 8.74 \\
\hline
\end{tabular}
\label{tab:results}
\end{center}
...
The lap time is dominated by the search, not by the racing. The spiral has to cover the arena before every gate is seen, and this takes on the order of 20~s on \texttt{final.toml}. The racing itself, once the gates are known, takes only a few seconds. Fig.~\ref{fig:speed} shows this split in the speed trace: a long low-speed sweep while the arena is covered, then a short fast burst through the four gates. This is why the search settings reach 18 to 27~s on a track that a known-track racer flies in under 10~s. The gain in return is generality. Because the search never reads the nominal layout, the same controller runs on any Level 3 draw, which is what the setting asks for.
...
That fast configuration, however, reads the gate positions and so only works when the layout is known. The open problem was the full Level 3 setting, where the layout is random. We therefore added the blind gate search in front of the same racing core. The search discovers the gates without the nominal positions, and the racing core then flies them. The submitted controller pays a fixed search cost on every track, including the graded one, in exchange for working on any Level 3 layout. We keep two settings of it, \texttt{v24\_r10} and the more aggressive \texttt{v24\_r10\_fast}, which is the same progression from safe to fast that we followed on the racing core. The aggressive real flight and the general search-based controller share the same racing engine, so the hardware result also validates the core of the submitted controller.
...
\section{Conclusion}
We presented a general Level 3 controller for autonomous drone racing under randomized layouts. It finds the gates with a blind spiral sweep that does not use the nominal positions, then races them with a time-optimal MPCC that makes speed an output. A gate-aware contouring weight and a dynamics-aware progress anchor keep the racing stage reliable. The \texttt{v24\_r10} setting finishes 12 of 20 runs on fully randomized Level 3 at 18.8~s, and the faster \texttt{v24\_r10\_fast} setting finishes 14 of 20 runs on the graded \texttt{final.toml} track at 18.4~s. Its racing core is the same one we flew on the real drone at 9.55~s in a fast known-track configuration. The design trades lap time for generality, which is the trade the full Level 3 setting demands.

\appendices
\section{Reproducibility}
The controller is evaluated on randomized Level 3 tracks and on the randomized \texttt{final.toml} competition track with its fixed seed, in the LSY Drone Racing environment, in attitude control mode at 50~Hz, with the Crazyflie 2.1 model, and requires the acados solver toolchain. The search parameters, the planner and the MPCC weights are shared across all tracks, and the submitted search controller uses no nominal layout. The controller code is available at \url{https://github.com/denizozturk95/lsy_drone_racing}.
