...
def _warp_waypoint(
    ...
    if gate_tag < 0:
        return wp.copy()
    nom_p = nominal_gates_pos[gate_tag]
    obs_p = observed_gates_pos[gate_tag]
    dyaw = float(observed_gates_yaw[gate_tag] - nominal_gates_yaw[gate_tag])
    c, s = float(np.cos(dyaw)), float(np.sin(dyaw))
    rel = wp - nom_p
    rot_rel = np.array([c * rel[0] - s * rel[1], s * rel[0] + c * rel[1], rel[2]])
    return obs_p + rot_rel
...
def warp_waypoints(
    ...
    out = np.zeros_like(waypoints)
    for i, (wp, tag) in enumerate(zip(waypoints, gate_tags)):
        out[i] = _warp_waypoint(
            wp, int(tag), NOMINAL_GATES_POS, NOMINAL_GATES_YAW, observed_gates_pos, observed_yaw
        )
    return out
...
def _first_reachable_index(
    ...
    for i, tag in enumerate(gate_tags):
        if tag == target_gate - 1:
            last_prev = i
    if last_prev < 0:
        return 0
    return min(last_prev + 1, len(warped) - 1)
...
def _feasible_segment_times(
    ...
    seg_t = np.maximum(seg_len / cfg.v_cruise, cfg.t_min_seg)
    seg_t[0] = max(seg_t[0], TAKEOFF_SEG_MIN_T)
    for _ in range(8):
        t = np.concatenate([[0.0], np.cumsum(seg_t)])
        spline = PchipInterpolator(t, waypoints, axis=0)
        dspline = spline.derivative(1)
        d2spline = spline.derivative(2)
        any_changed = False
        for i in range(len(seg_t)):
            ts = np.linspace(t[i], t[i + 1], 20)
            v_peak = float(np.max(np.linalg.norm(dspline(ts), axis=1)))
            a_peak = float(np.max(np.linalg.norm(d2spline(ts), axis=1)))
            util_v = v_peak / cfg.max_vel
            util_a = np.sqrt(a_peak / cfg.max_accel)
            util = max(util_v, util_a)
            target = 0.85  # leave ~15% margin for MPC tracking
            scale = float(np.clip(util / target, 0.85, 1.25))
            new_t = max(seg_t[i] * scale, cfg.t_min_seg)
            if i == 0:
                new_t = max(new_t, TAKEOFF_SEG_MIN_T)
            if abs(new_t - seg_t[i]) / max(seg_t[i], 1e-6) > 0.02:
                any_changed = True
            seg_t[i] = new_t
        if not any_changed:
            break
...
def build_racing_line_plan(
    ...
    warped = warp_waypoints(LEVEL2_WAYPOINTS, WAYPOINT_GATE_TAG, gates_pos, gates_quat)
    first = _first_reachable_index(warped, WAYPOINT_GATE_TAG, max(target_gate, 0))
    ...
    skip_first = float(np.linalg.norm(warped[first] - start_pos)) < 0.15
    tail = warped[first + (1 if skip_first else 0) :]
    wps = np.vstack([start_pos[None, :], tail])
    ...
    if len(wps) >= 2:
        for i in range(1, len(wps) - 1):
            ...
            t_n = float(np.linalg.norm(tangent[:2]))
            if t_n < 1e-6:
                continue
            lateral = np.array([-tangent[1] / t_n, tangent[0] / t_n, 0.0])
            wps[i] = _nudge_lateral(wps[i], lateral, obstacles_pos, cfg.r_obs)
    # Add midpoint detours for any segment that still grazes an obstacle
    wps = _insert_obstacle_midpoints(wps, obstacles_pos, cfg.r_obs)

    seg_t = _feasible_segment_times(wps, cfg)
    t_knots = np.concatenate([[0.0], np.cumsum(seg_t)])
    ...
    if float(np.linalg.norm(start_vel)) < 0.05:
        spline = PchipInterpolator(t_knots, wps, axis=0)
    else:
