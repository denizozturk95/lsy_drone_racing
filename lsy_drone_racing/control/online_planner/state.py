"""observation parsing routines for the controller."""
# ruff: noqa: TC002
...
class DroneObservation:
    """single-drone observation frame using quaternion order qx qy qz qw."""
...
def parse_observation(obs: dict[str, NDArray[np.floating]]) -> DroneObservation:
    ...
    if len(gate_pos) < 1 or len(gate_pos) != len(gate_quat):
        raise ValueError("expected at least one gate with matching quaternions")
