...
The shared online_planner/ planner gains ONE backward-compatible option used here:
``PlannerSettings.orient_gates_to_travel`` (default True = unchanged Level-2 behavior).
GateSearchV2 sets it False for NAVIGATE so gates are crossed along their canonical +x axis
...
*backwards* and the pass was never counted. controller_v6 (Level 2) uses the default and is
...
from lsy_drone_racing.control.online_planner.trajectory import ReferencePlan, ReferenceManager
...
class GateSearchV2(Controller):
    ...
    def __init__(self, obs: dict, info: dict, config: dict):
        ...
        if config.env.control_mode != "attitude":
            raise ValueError("GateSearchV2 requires env.control_mode = 'attitude'.")
