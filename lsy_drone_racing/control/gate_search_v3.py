...
# longer run-in reduces corner-cutting at the gate plane. r_obs is the obstacle keep-out
# radius (thin poles + drone radius need ~0.07 m; keep it tight so the crossing stays near
# the opening centre rather than being shoved toward the far frame bar).
_NAV_D_PRE = 0.60
_NAV_D_POST = 0.40
_NAV_R_OBS = 0.12
...
class GateSearchV3(Controller):
    ...
    def _project(self, curve, t_total: float, pos: NDArray) -> float:
